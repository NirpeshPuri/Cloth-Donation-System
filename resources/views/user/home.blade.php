...
@section('content')
                    ...
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($nearbyAdmins as $admin)
                            <div class="bg-white rounded-xl shadow-md overflow-hidden hover-scale cursor-pointer transition-all border-2 border-transparent hover:border-teal-500 relative"
                                ...
                                <!-- Medium Banner Image with fixed aspect ratio -->
                                <div class="relative overflow-hidden bg-gradient-to-r from-teal-500 to-emerald-500">
                                    <!-- Fixed container with 16:9 aspect ratio -->
                                    ...
                                    <!-- Distance Badge (absolute positioned) -->
                                    <div
                ...
                @if ($clothes->count() > 0)
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        @foreach ($clothes as $cloth)
                            <div class="bg-white rounded-xl shadow-md overflow-hidden cloth-card transition cursor-pointer"
                                onclick="window.location.href='{{ route('user.cloth.detail', $cloth->id) }}'">
                                <!-- Cloth Image with fixed aspect ratio -->
                                <div class="relative overflow-hidden bg-gray-200">
                                    <div class="aspect-square w-full">
                                        @if ($cloth->image)
                                            <img src="{{ Storage::url($cloth->image) }}" alt="{{ $cloth->name }}"
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-gray-500 text-sm mb-2">
                                        <i class="fas fa-tag mr-1"></i> {{ $cloth->category ?? 'Clothing' }}
                                    </p>
                                    <p class="text-gray-600 text-sm mb-3 line-clamp-2">
                                        {{ Str::limit($cloth->description, 80) }}
                                    </p>
                                    ...
                                    <button onclick="event.stopPropagation(); requestCloth({{ $cloth->id }})"
                                        class="w-full mt-4 bg-teal-600 hover:bg-teal-700 text-white py-2 rounded-lg transition font-semibold"
                                        {{ $cloth->quantity < 1 ? 'disabled' : '' }}>
                                        <i class="fas fa-hand-holding-heart mr-2"></i> Request This Item
                                    </button>
                                </div>
                            </div>
                        @endforeach
    ...
    @push('scripts')
        <script>
            ...
            function requestCloth(clothId) {
                event.stopPropagation();
                if (confirm('Would you like to request this item?')) {
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = '{{ route('user.request.cloth') }}';
                    form.innerHTML = `
                        @csrf
                        <input type="hidden" name="cloth_id" value="${clothId}">
                    `;
                    document.body.appendChild(form);
                    form.submit();
                }
