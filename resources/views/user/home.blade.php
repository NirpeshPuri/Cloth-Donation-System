...
@section('content')
    <div class="container mx-auto px-6 py-2">
        ...
        @if (!isset($selectedAdmin))
            <div class="gradient-bg rounded-2xl p-4 text-white mb-2">
        ...
        @if (!session()->has('user_latitude') && !Auth::user()->latitude && !isset($selectedAdmin))
        ...
        <!-- Stats Cards - Hidden when admin is selected -->
        @if (!isset($selectedAdmin))
            <div class="grid md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl p-4 shadow-md">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-2xl font-bold text-teal-600">{{ $cartCount ?? 0 }}</p>
                            <p class="text-xs text-gray-500">My Cart</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @if (isset($selectedAdmin))
                ...
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-lg bg-teal-200 flex items-center justify-center overflow-hidden">
                        @if ($selectedAdmin->profile_photo)
                            <img src="{{ Storage::url($selectedAdmin->profile_photo) }}" class="w-full h-full object-cover">
                        @else
            ...
            <!-- Search Button and Filters Section (Only when admin is selected) -->
            <div class="mb-8 flex justify-end">
                <button id="toggleSearchBtn" onclick="toggleSearch()"
                    class="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-lg transition flex items-center gap-2 shadow-md hover:shadow-lg">
                    <i class="fas fa-search"></i>
                    <span id="searchBtnText">Search Clothes</span>
                    <i id="searchBtnIcon" class="fas fa-chevron-down ml-2"></i>
                </button>
            </div>

            <!-- Search and Filters Section (Hidden by default) -->
        ...
        @if (isset($selectedAdmin))
            ...
            <!-- Original Sections (Only when admin is selected and search is not active) -->
            <div id="originalSections">
                    ...
                    <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800 flex items-center">
                                <i class="fas fa-fire text-orange-500 mr-3"></i> Most Request
                            </h2>
                        ...
                        <div class="relative">
                            <div class="scroll-container" id="scroll-popular">
                                @foreach ($popularItems as $cloth)
                                    @php
                                        $isInCart = in_array($cloth->id, $cartItemIds ?? []);
                                    @endphp
                                    <div class="product-card">
                                        ...
                                        <div class="p-4">
                                            ...
                                            <div class="flex justify-between items-center mt-2">
                                                ...
                                                <span
                                                    class="text-xs {{ $cloth->quantity <= 3 ? 'quantity-low' : ($cloth->quantity <= 10 ? 'quantity-medium' : 'quantity-high') }}">
                                                    {{ $cloth->quantity }} left
                                                </span>
                                            </div>
                                        </div>
                                        <div class="product-actions">
                                            <button onclick="viewDetails({{ $cloth->id }})" class="btn-view">
                                                <i class="fas fa-eye"></i> View
                                            </button>
                                            <button onclick="addToCart({{ $cloth->id }}, {{ $cloth->quantity }})"
                                                class="btn-cart" id="cart-btn-{{ $cloth->id }}"
                                                data-in-cart="{{ $isInCart ? 'true' : 'false' }}"
                                                {{ $cloth->quantity <= 0 ? 'disabled' : '' }}>
                ...
                <!-- Repeat for all other sections with the same pattern -->
                            ...
                            <div class="scroll-container" id="scroll-shoes">
                                @foreach ($categoryGroups['shoes'] as $cloth)
                                    ...
                                    <div class="product-card">
                                        ...
                                        <div class="p-4">
                                            <h3 class="font-bold text-gray-800">{{ $cloth->name }}</h3>
                                            <p class="text-sm text-gray-500">Size: {{ $cloth->size ?? 'Various' }}</p>
                                            <div class="flex justify-between items-center mt-2">
                                                <span
                                                    class="text-xs bg-teal-100 text-teal-600 px-2 py-1 rounded-full">{{ $cloth->gender ?? 'Unisex' }}</span>
                                                <span
        ...
        <script>
            ...
            function toggleSearch() {
                ...
                if (searchVisible) {
                    ...
                    searchBtnText.textContent = 'Search Clothes';
                    searchBtnIcon.className = 'fas fa-chevron-down ml-2';
                    document.getElementById('originalSections').style.display = 'block';
                } else {
                    searchSection.style.display = 'block';
                    searchBtnText.textContent = 'Hide Search';
                    searchBtnIcon.className = 'fas fa-chevron-up ml-2';
                    document.getElementById('originalSections').style.display = 'none';
                }
                searchVisible = !searchVisible;
            }

            // ========== INITIALIZE CART BUTTONS - INSTANT ==========
                ...
                fetch('{{ route('cart.add') }}', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            btn.classList.add('in-cart');
                            btn.querySelector('.btn-text').innerHTML = '<i class="fas fa-check"></i> In Cart';
                            btn.disabled = true;
                    ...
                    .then(data => {
                        ...
                        if (data.searches && data.searches.length > 0) {
                            ...
                            data.searches.forEach(search => {
                                html +=
                                    `<button onclick="applyRecentSearch('${search.replace(/'/g, "\\'")}')" class="recent-search-tag">${search}</button>`;
                            });
            ...
            function displaySearchResults(data) {
                ...
                // Get cart items from PHP data
                const cartItemIds = @json($cartItemIds ?? []);

                let html = '';
                data.items.forEach(cloth => {
                    const isInCart = cartItemIds.includes(cloth.id);
                    html += `
                        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition">
                            <div class="relative h-48 overflow-hidden cursor-pointer" onclick="viewDetails(${cloth.id})">
                                ${cloth.image_path ?
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex justify-between items-center mt-2">
                                    ...
                                    <span class="text-xs ${cloth.quantity <= 3 ? 'quantity-low' : (cloth.quantity <= 10 ? 'quantity-medium' : 'quantity-high')}">
                                        ${cloth.quantity} available
                                    </span>
                                </div>
                                <div class="mt-2 flex items-center gap-2">
                ...
                // Initialize cart buttons for search results
            ...
            // ========== INITIALIZE ON PAGE LOAD - INSTANT ==========
            document.addEventListener('DOMContentLoaded', function() {
                updateSizeOptions();
                loadRecentSearches();
                // Initialize cart buttons instantly - no delay!
                initializeCartButtons();
            });
        </script>
    @endpush
                ...
                fetch('{{ route('user.recent-searches') }}', {
                    ...
                    .then(data => {
                        ...
                        if (data.searches && data.searches.length > 0) {
                            ...
                            data.searches.forEach(search => {
                                html +=
                                    `<button onclick="applyRecentSearch('${search.replace(/'/g, "\\'")}')" class="recent-search-tag">
                                ${search}
                            </button>`;
                            });
                            container.innerHTML = html;
